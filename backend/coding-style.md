...
- 不要删除已有业务逻辑和已有注释。
- 修改代码优先只处理当前问题相关逻辑。
- 补齐当前文件全部 JavaDoc、拆分单行代码和进行必要格式整理不视为无关修改。
...
在输出或提交 Java 代码前，必须逐项检查当前修改过的每个 Java 文件：

- 类型定义是否都有业务用途说明、`@author NPC` 和 `@date`。
- 是否误写成了 `@data`。
- 普通类、接口、枚举、Record、注解类型和内部类型是否全部检查。
- 所有字段和常量是否都有多行 JavaDoc。
- 所有 `private final` 依赖字段是否都有 JavaDoc。
- 所有显式构造方法是否都有 JavaDoc 和完整 `@param`。
- 所有方法是否都有 JavaDoc，包括 Controller 方法、私有方法、静态方法、转换方法、工厂方法和重写方法。
- 非 `void` 方法是否都有 `@return`。
- 每个方法参数是否都有对应的 `@param`。
- 重要异常是否有 `@throws`。
- Record 是否有类型 JavaDoc和每个组件对应的 `@param`。
- 关键下游调用前是否有说明业务目的的普通行注释。
- 是否存在单行字段 JavaDoc、单行构造方法或单行方法体。
- 是否改变了与本次需求无关的业务逻辑。
- 用户要求完整代码时，输出是否完整且没有使用省略号。
- 是否使用 MyBatis-Plus 的实体注解与 Mapper，且未引入 JPA 相关类型。
- 实体类是否使用且仅使用 `@Data` 管理访问器，未手写 Getter、Setter、`equals`、`hashCode`、`toString`。
- 是否存在内部类、内部接口、内部枚举或内部 Record；如存在，是否已拆分为独立顶级文件。
